...
class StateStore:
    ...
    def get_agent(self, name: str) -> dict[str, Any]:
        agents = self.agents
        return next(a for a in agents if a.get("name") == name)

    def count_agents(self, name: str) -> int:
        return sum(1 for a in self.agents if a.get("name") == name)

    def update_agent_status(
        self, name: str, status: Literal["in_progress", "completed"]
    ) -> None:
    ...
    def set_plan_review_scores(self, scores: dict[str, int]) -> None:
        def _set(d: dict) -> None:
            plan = d.setdefault("plan", {})
            review = plan.setdefault("review", {})
    ...
    def set_code_review_scores(self, scores: dict[str, int]) -> None:
        def _set(d: dict) -> None:
            cf = d.setdefault("code_files", {})
            review = cf.setdefault("review", {})
            review["scores"] = scores
