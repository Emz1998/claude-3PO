...
def is_webfetch_allowed(state: StateStore, hook_input: dict, config: Config) -> str:
    """Validate that a WebFetch URL targets a safe domain."""
    tool_name = hook_input.get("tool_name", "")
    if tool_name != "WebFetch":
        return "Skipping, not a WebFetch tool"

    url = hook_input.get("tool_input", {}).get("url", "")
    return is_safe_domain(url, config)


def is_command_allowed(hook_input: dict, config: Config, state: StateStore) -> str:
    ...
    if tool_name != "Bash":
        return "Skipping, not a Bash tool"
...
def is_file_write_allowed(hook_input: dict, config: Config, state: StateStore) -> str:
    ...
    if tool_name != "Write":
        return "Skipping, not a Write tool"
...
def is_file_edit_allowed(hook_input: dict, config: Config, state: StateStore) -> str:
    """Validate the code files. If extensions are not supported, raise an error."""
    phase = state.current_phase
    tool_name = hook_input.get("tool_name", "")
    tool_input = hook_input.get("tool_input", {})
...
def is_agent_allowed(hook_input: dict, config: Config, state: StateStore) -> str:
    ...
    return f"{next_agent} agent is allowed in phase: {phase}"
