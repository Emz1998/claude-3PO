...
def main() -> int:
    ...
    ap.add_argument("--repo", help="owner/repo — also update tasks.json if --tasks is given")
    ap.add_argument("--tasks", help="Path to tasks.json to keep in sync")
    ap.add_argument("--list-statuses", action="store_true", help="List available statuses and exit")
    args = ap.parse_args()
    ...
    run([
        ...
        "--single-select-option-id", options[args.status],
    ])
