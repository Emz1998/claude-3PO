...
class PhaseEngine:
    ...
    def can_bypass_to(self, current_phase: str, bypass_phase: str) -> tuple[bool, str]:
        ...
        if current_phase in config.cannot_bypass:
            return (
                False,
                f"Cannot enter {bypass_phase} from '{current_phase}' (pre-coding phase)",
            )
    ...
    def is_valid_transition(
        ...
        if next_idx < current_idx:
            return (
                False,
                f"Cannot go backwards from '{current_phase}' to '{next_phase}'",
            )
...
def is_valid_transition(current_phase: str | None, next_phase: str) -> tuple[bool, str]:
    """Validate a phase transition."""
...
if __name__ == "__main__":
    ...
    print(
        f"Valid transition explore -> plan: {engine.is_valid_transition('explore', 'plan')}"
    )
    print(
        f"Invalid transition explore -> commit: {engine.is_valid_transition('explore', 'commit')}"
    )
