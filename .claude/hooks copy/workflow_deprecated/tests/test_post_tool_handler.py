...
def handler():
    ...
    with patch("handlers.post_tool.get_manager") as mock_manager, patch(
        "handlers.post_tool.PhaseTracker"
    ) as mock_phase, patch(
        "handlers.post_tool.DeliverableTracker"
    ) as mock_deliverable, patch(
        "handlers.post_tool.ReleasePlanTracker"
    ) as mock_release, patch(
        "handlers.post_tool.ContextInjector"
    ) as mock_context:
...
def inactive_handler():
    ...
    with patch("handlers.post_tool.get_manager") as mock_manager, patch(
        "handlers.post_tool.PhaseTracker"
    ), patch("handlers.post_tool.DeliverableTracker"), patch(
        "handlers.post_tool.ReleasePlanTracker"
    ), patch(
        "handlers.post_tool.ContextInjector"
    ):
