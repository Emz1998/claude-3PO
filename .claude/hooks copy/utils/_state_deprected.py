...
def load_state(file_path: Path = CACHE_PATH) -> dict:
    try:
...
def get_state(key: str, state_path: Path = CACHE_PATH) -> Any:
...
def write_state(cache: dict, cache_path: Path = CACHE_PATH) -> None:
    """Write cache to file."""
    cache_path.write_text(json.dumps(cache, indent=2))


def set_state(key: str, value: Any, cache_path: Path = CACHE_PATH) -> None:
    ...
    if not key:
        ...
        return
    cache[key] = value
    write_state(cache, cache_path)


def append_state(key: str, value: Any, cache_path: Path = CACHE_PATH) -> None:
