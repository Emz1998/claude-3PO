...
STATE_PATH = PROJECT_ROOT / "project/sprints/SPRINT-001/sprint.json"
...
class SprintConfig:
    ...
    def get_story_type(self, story_id: str) -> str:
        """Get the story type.
    ...
    def get_story_list(self) -> list[str]:
        ...
        """
        stories = self.get("stories", [])
        return [story.get("id") for story in stories]

    def get_story_deps(self, story_id: str) -> list[str]:
        """Get the dependencies of a story.
        ...
        if story is None:
            return []
        return story.get("dependsOn", [])

    def get_story_with_deps(self) -> list[str]:
        """Get the stories with dependencies.

        Returns:
            The stories with dependencies
        """
        stories = self.get("stories", [])
        return [story.get("id") for story in stories if story.get("dependsOn")]

    def get_stories_without_deps(self) -> list[str]:
        """Get the stories without dependencies.

        Returns:
            The stories without dependencies
        """
        stories = self.get("stories", [])
        return [story.get("id") for story in stories if not story.get("dependsOn")]

    def find_task(self, story_id: str, task_id: str) -> dict[str, Any] | None:
        """Get the task by id.
    ...
    def get_task_list(self, story_id: str) -> list[str]:
        """Get the task list for a story.
        ...
        if story is None:
            return []
        return [task.get("id") for task in story.get("tasks", [])]

    def get_tasks_with_deps(self, story_id: str) -> list[str]:
        """Get the tasks with dependencies.
        ...
        if story is None:
            return []
        tasks = story.get("tasks", [])
        return [task.get("id") for task in tasks if task.get("dependsOn")]

    def get_tasks_without_deps(self, story_id: str) -> list[str]:
        """Get the tasks without dependencies.

        Returns:
            The tasks without dependencies
        """
        story = self.find_story(story_id)
        if story is None:
            return []
        tasks = story.get("tasks", [])
        return [task.get("id") for task in tasks if not task.get("dependsOn")]

    def get_task_deps(self, story_id: str, task_id: str) -> list[str]:
        """Get the dependencies of a task.

        Args:
            task_id: The task id
        """
        task = self.find_task(story_id, task_id)
        if task is None:
            return []
        return task.get("dependsOn", [])
