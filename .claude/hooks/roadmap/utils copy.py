...
def load_roadmap(
    roadmap_path: Path = get_roadmap_path(get_current_version()),
) -> dict | None:
...
def get_task_status(task_id: str) -> StatusType | None:
...
def get_owners_with_completed_deps(roadmap: dict, milestone: dict) -> list[str]:
    """Get owners of tasks whose dependencies are all completed within a milestone."""
    owners = []
    for task in milestone.get("tasks", []):
        incomplete = get_incomplete_task_deps(roadmap, task)
        if not incomplete:  # all deps completed
            owners.append(get_task_owner(task))
...
def get_current_milestone_tasks() -> list[dict]:
    """Get the tasks in the current milestone."""
    current_milestone_id = get_current_milestone_id() or ""
    return get_milestone_tasks(load_roadmap() or {}, current_milestone_id)


def are_tasks_completed_in_current_milestone() -> bool:
    """Check if all tasks in a milestone are completed."""
    return all(
        task.get("status") == "completed" for task in get_current_milestone_tasks()
    )


def get_current_milestone_parallel_agents() -> list[str]:
    """Get the valid task owners for the current milestone."""
    tasks_in_milestone = get_current_milestone_tasks()
    task_owners = []
    print(json.dumps(tasks_in_milestone, indent=2))
    for task in tasks_in_milestone:

        print(task.get("status"))
        if task.get("status") != "completed":
            for task_dep in task.get("dependencies", []):
                task_completed = (
                    get_task_status(task_dep) == "completed" if task_dep else False
                )
                if task_completed:
                    task_owners.append(get_task_owner(task))

    return list(set(task_owners))
