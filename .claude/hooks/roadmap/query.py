...
def query_version(roadmap: dict) -> str:
    ...
    lines.append(
        f"  Phases: {phases.get('completed', 0)}/{phases.get('total', 0)} completed"
    )
    lines.append(
        f"  Milestones: {milestones.get('completed', 0)}/{milestones.get('total', 0)} completed"
    )
    lines.append(
...
def query_current(roadmap: dict) -> str:
    ...
    if phase_id:
        phase = find_phase_in_roadmap(roadmap, phase_id)
        if phase:
    ...
    for phase in roadmap.get("phases", []):
        for ms in phase.get("milestones", []):
            ...
            for task in ms.get("tasks", []):
                ...
                lines.append(
                    f"  {status} [{task.get('id')}] {task.get('description', 'Unknown')}"
                )
                lines.append(f"      Owner: {task.get('owner', 'Unknown')}")
                lines.append(f"      ACs: {ac_met}/{len(acs)} met")
                if task.get("dependencies"):
                    lines.append(
                        f"      Deps: {', '.join(task.get('dependencies', []))}"
                    )
    return "\n".join(lines)
...
def query_milestone_detail(roadmap: dict, milestone_id: str) -> str:
    ...
    for task in tasks:
        ...
        lines.append(
            f"      Owner: {task.get('owner', 'Unknown')}, ACs: {ac_met}/{len(acs)}"
        )
...
def query_ac_detail(roadmap: dict, ac_id: str) -> str:
    ...
    # Get phase and milestone context
    phase = None
    milestone = None
    if task:
        phase, milestone, _ = find_task_in_roadmap(roadmap, task.get("id", ""))

    lines = [
        f"Acceptance Criteria: [{ac_id}]",
        f"Status: {format_status(ac.get('status', 'unknown'))}",
        (
            f"Task: [{task.get('id')}] {task.get('description', 'Unknown')}"
            if task
            else ""
        ),
        (
...
def query_sc_detail(roadmap: dict, sc_id: str) -> str:
    ...
    phase = None
    if milestone:
        phase, _ = find_milestone_in_roadmap(roadmap, milestone.get("id", ""))
            ...
            for task in ms.get("tasks", []):
                if task.get("status") == "blocked":
                    blocked_tasks.append(
                        (ms.get("id"), task.get("id"), task.get("description"))
                    )
...
def query_todo(roadmap: dict) -> str:
    ...
    phase = find_phase_in_roadmap(roadmap, current_phase_id)
    if not phase:
    ...
    lines.append("=" * 60)
    lines.append(
        f"Summary: {completed} completed, {in_progress} in progress, {not_started} pending"
    )
...
Examples:
  ...
  %(prog)s blockers         # Show all blockers
    ...
    parser.add_argument("query", type=str, help="Query type or item ID")
    parser.add_argument(
        "filter_id",
        type=str,
        nargs="?",
