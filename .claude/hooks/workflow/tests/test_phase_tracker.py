...
def tracker():
    ...
    with patch("trackers.phase_tracker.get_manager") as mock_manager, \
         patch("trackers.phase_tracker.get_tracker") as mock_deliverables:
...
class TestRun:
    ...
    def test_run_handles_skill_tool_post_use(self):
        ...
        with patch("trackers.phase_tracker.get_manager") as mock_manager, \
             patch("trackers.phase_tracker.get_tracker") as mock_deliverables, \
             patch("trackers.phase_tracker.normalize_skill_name") as mock_normalize, \
             patch("core.workflow_auditor.get_auditor") as mock_auditor:
            mock_state = MagicMock()
            mock_state.is_workflow_active.return_value = True
            mock_state.get_deliverables.return_value = []
