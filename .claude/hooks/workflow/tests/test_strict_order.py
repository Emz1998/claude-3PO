...
class TestStrictOrderBlocking:
    ...
    def test_allows_current_level(self, manager: StateManager):
        ...
        manager.set_deliverables([
            _make_deliverable(action="read", pattern=".*prompt\\.md$", strict_order=1),
            _make_deliverable(action="write", pattern=".*report\\.md$", strict_order=2),
        ])
    ...
    def test_blocks_non_strict_deliverable(self, manager: StateManager):
        ...
        manager.set_deliverables([
            _make_deliverable(action="read", pattern=".*prompt\\.md$", strict_order=1),
            _make_deliverable(action="write", pattern=".*notes\\.md$", strict_order=None),
        ])
    ...
    def test_unlocks_after_level_done(self, manager: StateManager):
        ...
        manager.set_deliverables([
            _make_deliverable(action="read", pattern=".*prompt\\.md$", strict_order=1, completed=True),
            _make_deliverable(action="write", pattern=".*report\\.md$", strict_order=2),
        ])
        result = manager.get_strict_order_block_reason("write", "/path/report.md")
        assert result is None
    ...
    def test_unlocks_all_after_strict_done(self, manager: StateManager):
        ...
        manager.set_deliverables([
            _make_deliverable(action="read", pattern=".*prompt\\.md$", strict_order=1, completed=True),
            _make_deliverable(action="write", pattern=".*report\\.md$", strict_order=2, completed=True),
            _make_deliverable(action="edit", pattern=".*other\\.md$", strict_order=None),
        ])
    ...
    def test_same_level_any_order(self, manager: StateManager):
        ...
        manager.set_deliverables([
            _make_deliverable(action="read", pattern=".*file_a\\.md$", strict_order=1),
            _make_deliverable(action="read", pattern=".*file_b\\.md$", strict_order=1),
        ])
    ...
    def test_no_strict_order_no_blocking(self, manager: StateManager):
        ...
        manager.set_deliverables([
            _make_deliverable(action="read", pattern=".*prompt\\.md$", strict_order=None),
            _make_deliverable(action="write", pattern=".*report\\.md$", strict_order=None),
        ])
