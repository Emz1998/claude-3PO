...
def handler():
    ...
    with patch("handlers.pre_tool.get_manager") as mock_manager, \
         patch("handlers.pre_tool.PhaseTransitionGuard") as mock_phase_guard, \
         patch("handlers.pre_tool.SubagentAccessGuard") as mock_subagent_guard, \
         patch("handlers.pre_tool.ReleasePlanTracker") as mock_tracker, \
         patch("handlers.pre_tool.get_current_feature_id") as mock_feature_id, \
         patch("handlers.pre_tool.get_feature_test_strategy") as mock_strategy:
...
class TestCheckStrictOrder:
    ...
    def test_check_strict_order_allows_when_no_block(self):
        ...
        with patch("handlers.pre_tool.get_manager") as mock_manager, \
             patch("handlers.pre_tool.PhaseTransitionGuard"), \
             patch("handlers.pre_tool.SubagentAccessGuard"), \
             patch("handlers.pre_tool.ReleasePlanTracker"), \
             patch("handlers.pre_tool.get_current_feature_id") as mock_feature_id, \
             patch("handlers.pre_tool.get_feature_test_strategy") as mock_strategy:
            mock_manager.return_value.is_workflow_active.return_value = True
            mock_manager.return_value.get_strict_order_block_reason.return_value = None
