...
class TestStrictOrderCompliance:
    ...
    def test_detects_level_skip(self, auditor: WorkflowAuditor):
        ...
        deliverables = [
            ...
            _make_deliverable(strict_order=3, completed=True, action="write", pattern=".*report\\.md$"),
        ]
        ...
        assert "level 3" in violations[0].message.lower() or "Level 3" in violations[0].message
        assert "level 1" in violations[0].message.lower() or "Level 1" in violations[0].message
    ...
    def test_multiple_violations(self, auditor: WorkflowAuditor):
        ...
        deliverables = [
            ...
            _make_deliverable(strict_order=3, completed=True, action="write", pattern=".*a$"),
            _make_deliverable(strict_order=4, completed=True, action="edit", pattern=".*b$"),
        ]
        violations = auditor.check_strict_order_compliance(deliverables)
