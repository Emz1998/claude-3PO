...
def guard():
    ...
    with patch("guards.deliverables_exit.get_manager") as mock_manager, \
         patch("guards.deliverables_exit.get_tracker") as mock_tracker:
    ...
    def test_validate_deliverables_returns_unmet_message(self):
        ...
        with patch("guards.deliverables_exit.get_manager") as mock_manager, \
            ...
            mock_tracker.return_value.are_all_met.return_value = (
                False, "Deliverable not met: write codebase-status_*.md"
            )
...
class TestValidateScs:
    ...
    def test_validate_scs_returns_true_when_import_fails(self):
        ...
        with patch("guards.deliverables_exit.get_manager") as mock_manager, \
             patch("guards.deliverables_exit.get_tracker") as mock_tracker, \
             patch.dict(sys.modules, {"roadmap.utils": None}):
            mock_manager.return_value.is_workflow_active.return_value = True
...
class TestRun:
    ...
    def test_run_blocks_when_deliverables_not_met(self, capsys):
        ...
        with patch("guards.deliverables_exit.get_manager") as mock_manager, \
             patch("guards.deliverables_exit.get_tracker") as mock_tracker, \
             patch("core.workflow_auditor.get_auditor") as mock_auditor:
            mock_manager.return_value.is_workflow_active.return_value = True
            mock_tracker.return_value.are_all_met.return_value = (
                False, "Missing deliverable: read codebase.md"
            )
    ...
    def test_run_allows_when_all_met(self):
        ...
        with patch("guards.deliverables_exit.get_manager") as mock_manager, \
            ...
            with patch.object(guard, "validate_scs", return_value=(True, "All SCs met")):
                with pytest.raises(SystemExit) as exc_info:
                    guard.run({"hook_event_name": "SubagentStop"})
    ...
    def test_run_blocks_when_scs_not_met(self, capsys):
        ...
        with patch("guards.deliverables_exit.get_manager") as mock_manager, \
            ...
            with patch.object(guard, "validate_scs", return_value=(False, "SC-001 not met")):
                with pytest.raises(SystemExit) as exc_info:
                    guard.run({"hook_event_name": "SubagentStop"})
    ...
    def test_validate_deliverables_exit_returns_true_when_all_met(self):
        ...
        with patch("guards.deliverables_exit.get_manager") as mock_manager, \
            ...
            with patch.object(guard, "validate_scs", return_value=(True, "All SCs met")):
                ...
                with patch("guards.deliverables_exit.DeliverablesExitGuard") as MockGuard:
                    mock_guard_instance = MagicMock()
                    mock_guard_instance.validate_deliverables.return_value = (True, "All met")
                    mock_guard_instance.validate_scs.return_value = (True, "All SCs met")
