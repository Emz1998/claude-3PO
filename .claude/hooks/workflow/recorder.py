...
def record_subagent_stop(hook_input: dict, store: StateStore) -> tuple[str, str]:
    ...
    def _process(state: dict) -> None:
        ...
        # Explore / Research: collect findings, check if all done -> advance to write-codebase
        if agent_type in ("Explore", "Research") and phase == "explore":
            # Store agent findings for CODEBASE.md generation
...
# ---------------------------------------------------------------------------
# ExitPlanMode recording (PostToolUse ExitPlanMode)
# ---------------------------------------------------------------------------
