...
class TestInject:
    ...
    def test_inject_calls_get_phase_reminder(self):
        ...
        with patch("context.context_injector.get_manager") as mock_manager, patch(
            "context.context_injector.get_phase_reminder"
        ) as mock_reminder, patch(
            "context.context_injector.add_context"
        ) as mock_add_context:
            mock_manager.return_value.is_workflow_active.return_value = True
            mock_manager.return_value.get_pending_validation.return_value = None
...
class TestInjectValidationContext:
    ...
    def test_inject_validation_context_when_pending(self):
        ...
        with patch("context.context_injector.get_manager") as mock_manager, patch(
            ...
            with patch.dict(
                sys.modules, {"validators.criteria_validator": MagicMock()}
            ):
                ...
                sys.modules["validators.criteria_validator"].get_unmet_epic_scs = (
                    MagicMock()
                )
...
class TestRun:
    ...
    def test_run_injects_for_skill_tool(self):
        ...
        with patch("context.context_injector.get_manager") as mock_manager, patch(
            ...
            "context.context_injector.add_context"
        ) as mock_add_context, patch(
            "context.context_injector.normalize_skill_name"
        ) as mock_normalize:
            mock_manager.return_value.is_workflow_active.return_value = True
            mock_manager.return_value.get_pending_validation.return_value = None
