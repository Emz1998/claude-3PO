    ...
    def run(self, hook_input: dict) -> None:
        ...
        if not self.is_subagent_allowed(current_phase, subagent):
            ...
            auditor.log_decision(
                "SUBAGENT_GUARD", "BLOCK", f"'{subagent}' in phase '{current_phase}'"
            )
            print(
        ...
        auditor.log_decision(
            "SUBAGENT_GUARD", "ALLOW", f"'{subagent}' in phase '{current_phase}'"
        )
        sys.exit(0)
...
def validate_subagent_access(phase: str, subagent: str) -> tuple[bool, str]:
    ...
    return (
        False,
        f'Subagent "{subagent}" not allowed in phase "{phase}". Expected: "{allowed}"',
    )
