...
class StateStore:
    ...
    #  ────────────────── Reviews ────────────────────────
    @property
    def _reviews(self) -> dict[Literal["plan", "test"], list[dict[str, Any]]]:
        return self.state.get("reviews", {})

    def get_reviews(self, review_type: Literal["plan", "test"]) -> list[dict[str, Any]]:
        return self._reviews.get(review_type, [])
    ...
    def update_review(
        self, review_type: Literal["plan", "test"], status: Status, verdict: Verdict
    ) -> None:
    ...
    def get_agents(self, status: Status | None = None) -> list[dict[str, Any]]:
        if status is None:
            return self.agents
    ...
    def is_agent_in_progress(self, name: str) -> bool:
        return any(
            agent["name"] == name for agent in self.get_agents(status="in_progress")
        )

    def is_agent_completed(self, name: str) -> bool:
        return any(
            agent["name"] == name for agent in self.get_agents(status="completed")
        )
