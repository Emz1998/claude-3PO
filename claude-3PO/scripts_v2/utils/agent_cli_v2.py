...
def build_codex_argv(prompt: str, options: CodexConfig) -> list[str]:
    ...
    # codex exec accepts [PROMPT] as a trailing positional; pass it directly so
    # callers don't have to pipe stdin (which subprocess.Popen wouldn't supply).
    argv.append(prompt)
    return argv
...
def run_headless_parallel(commands) -> tuple[bool, list[str]]:
