"""Tests for headless/codex/codex_plan_review.py — plan-review orchestration.

The subprocess contract lives in ``lib.shell.invoke_codex`` and is covered by
``test_shell.py``. These tests patch ``invoke_codex`` at the import site so
...
from headless.codex.codex_plan_review import (
...
PATCH_TARGET = "headless.codex.codex_plan_review.invoke_codex"
...
class TestInvokeCodexPlanReview:
    ...
    def test_returns_none_when_plan_missing(self, tmp_path):
        ...
        with patch(PATCH_TARGET) as inv:
            out = invoke_codex_plan_review(missing, timeout=30)
        assert out is None
