...
The hook validates that subtask subjects match a project task in the
implement workflow and that descriptions are non-empty.
"""
...
class TestImplementTaskCreatedMatching:
    ...
    def test_empty_subject_blocked(self, implement_state_path):
        result = _run_hook(_task_payload(""), implement_state_path)
        assert result.returncode == 2

    def test_empty_description_blocked(self, implement_state_path):
        result = _run_hook(_task_payload("Build login", ""), implement_state_path)
        assert result.returncode == 2
...
class TestTaskCreatedWorkflowInactive:
    def test_inactive_workflow_allows_all(self, tmp_path):
        inactive_state = dict(IMPLEMENT_STATE)
