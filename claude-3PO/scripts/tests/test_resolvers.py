...
class TestResolveInstallDependencies:
    def test_completes_when_installed(self, config, state):
        state.add_phase("install-deps")
        state.set_dependencies_installed()
        Resolver(config, state)._resolve_install_deps()
        assert state.is_phase_completed("install-deps")

    def test_does_not_complete_when_not_installed(self, config, state):
        state.add_phase("install-deps")
        Resolver(config, state)._resolve_install_deps()
        assert not state.is_phase_completed("install-deps")

    def test_does_not_complete_with_packages_but_not_installed(self, config, state):
        state.add_phase("install-deps")
...
class TestResolveDefineContracts:
    def test_completes_when_written_and_validated(self, config, state):
        state.add_phase("define-contracts")
        state.set_contracts_written(True)
        state.set_contracts_validated(True)
    ...
    def test_does_not_complete_when_not_validated(self, config, state):
        state.add_phase("define-contracts")
        state.set_contracts_written(True)
        Resolver(config, state)._resolve_define_contracts()
        assert not state.is_phase_completed("define-contracts")

    def test_does_not_complete_when_neither(self, config, state):
        state.add_phase("define-contracts")
        Resolver(config, state)._resolve_define_contracts()
        assert not state.is_phase_completed("define-contracts")
