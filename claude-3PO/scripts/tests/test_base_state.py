...
1. :class:`BaseState` instantiates standalone (no facade required) and its
   shared-method surface round-trips through the JSONL file.
...
class TestBaseStateStandalone:
    ...
    def test_instantiation_standalone(self, tmp_path: Path):
        ...
        base = BaseState(path, session_id="s1")
    ...
    def test_shared_method_round_trip(self, tmp_path: Path):
        ...
        path = tmp_path / "state.jsonl"
        base = BaseState(path, session_id="s2", default_state={"foo": 0})
        base.set("foo", 42)
...
class TestSharedFileAndLock:
    ...
    def test_slices_share_base_reference(self, tmp_path: Path):
        ...
        store = StateStore(tmp_path / "state.jsonl", session_id="s")
    ...
    def test_single_jsonl_line_per_session(self, tmp_path: Path):
        # Two writes through the facade should produce one line on disk.
        path = tmp_path / "state.jsonl"
        store = StateStore(path, session_id="s")
        store.set("a", 1)
        store.set("b", 2)
