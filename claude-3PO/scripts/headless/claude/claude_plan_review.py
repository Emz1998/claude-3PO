...
PROMPT_PATH = (
    ...
    / "headless" / "prompts" / "claude" / "plan_review.md"
)
...
def _build_prompt(plan_text: str) -> str:
    ...
    Returns:
        str: Review prompt ready to send to claude.
...
def invoke_claude_plan_review(
    plan_path: Path,
    timeout: int = 120,
    cwd: Path | None = None,
) -> str | None:
    """
    Ask headless Claude to review the plan at *plan_path*.

    Reads the plan, builds the review prompt, and delegates to
    :func:`lib.shell.invoke_claude`. Returns ``None`` if the plan file does
    ...
    Args:
        ...
        timeout (int): Max seconds for the claude subprocess. Defaults to 120.
        cwd (Path | None): Working directory for claude; ``None`` uses current.

    Returns:
        str | None: Reviewer message on success, ``None`` on any failure.

    Example:
        >>> invoke_claude_plan_review(Path("plan.md"))  # doctest: +SKIP
    """
    if not plan_path.is_file():
        return None
    prompt = _build_prompt(plan_path.read_text(encoding="utf-8"))
    return invoke_claude(prompt, timeout=timeout, cwd=cwd)
