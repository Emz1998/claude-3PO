...
from lib.subprocess_agents import invoke_agent, InvokeConfig  # type: ignore
from utils.prompt_retriever import retrieve_prompt
...
class Research:
    ...
    def invoke_researcher(self, llm: Literal["codex", "claude"]) -> str:
        # Default model per flavor; session_id pin comes from the current hook payload.
        ...
        return invoke_agent(retrieve_prompt("research"), config)

    def setup_webfetch_guard(self) -> Decision:
        guard = WebFetchGuard(self.hook_input, self.config)
        return guard.validate()

    def run(self, guard_type: Literal["webfetch", "template_check"]) -> Decision:
