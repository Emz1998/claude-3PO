...
from lib.paths import all_revised
...
class AgentGuard:
    ...
    entry point; ``check_*`` helpers raise ``ValueError`` and ``validate`` catches
    them and converts to ``("block", reason)``.
    ...
    def check_plan_revision_done(self) -> None:
        """
    ...
    def check_test_revision_done(self) -> None:
        ...
        Example:
            ...
            >>> guard.check_test_revision_done()  # doctest: +SKIP
        """
        last = self.state.last_test_review
        # Skip when the last review passed — there's nothing to revise.
    ...
    def check_code_revision_done(self) -> None:
        """
        ...
        Example:
            ...
            >>> guard.check_code_revision_done()  # doctest: +SKIP
        """
    ...
    def validate(self) -> Decision:
        ...
        agent/count/revision checks. Any ``ValueError`` raised by a check
        becomes a Block.
