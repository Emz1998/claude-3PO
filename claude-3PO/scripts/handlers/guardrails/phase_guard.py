...
class PhaseGuard:
    ...
    - **Special skills** — ``/continue``, ``/plan-approved``, ``/revise-plan``,
      and ``/reset-plan-review`` each have their own ``handle_*`` method
    ...
    def is_review_exhausted(self, phase: str) -> bool:
        """
        ...
        Example:
            >>> guard.is_review_exhausted("plan-review")  # doctest: +SKIP
            False
        """
        # QA phases share one counter pair, so they branch off before the
        # per-phase table lookup.
    ...
    def handle_plan_approved(self) -> Decision:
        """
        ...
        Example:
            >>> decision, message = guard.handle_plan_approved()  # doctest: +SKIP
        """
        if self.current != "plan-review":
            raise ValueError(
        ...
        if self.status == "in_progress" and self.is_review_exhausted("plan-review"):
            return (
                "allow",
    ...
    def handle_revise_plan(self) -> Decision:
        """
        ...
        Example:
            >>> decision, message = guard.handle_revise_plan()  # doctest: +SKIP
        """
        if self.current != "plan-review":
            raise ValueError(
        ...
        is_exhausted = self.status == "in_progress" and self.is_review_exhausted(
            "plan-review"
        )
    ...
    def handle_reset_plan_review(self) -> Decision:
        """
        ...
        Example:
            >>> decision, message = guard.handle_reset_plan_review()  # doctest: +SKIP
        """
    ...
    def check_clarify_iteration_ceiling(self) -> Decision:
        """
        ...
        Example:
            >>> decision, message = guard.check_clarify_iteration_ceiling()  # doctest: +SKIP
        """
    ...
    def validate(self) -> Decision:
        ...
        if self.tool_name == "AskUserQuestion":
            return self.check_clarify_iteration_ceiling()
        try:
            # Operator skills bypass the ordering check — each has its own
            # phase/status preconditions that the handler enforces.
            if self.next_phase == "continue":
                return self.handle_continue()
            if self.next_phase == "plan-approved":
                return self.handle_plan_approved()
            if self.next_phase == "revise-plan":
                return self.handle_revise_plan()
            if self.next_phase == "reset-plan-review":
                return self.handle_reset_plan_review()
