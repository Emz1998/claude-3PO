...
def build_correction_prompt(diff: str, mock_report: bool = True) -> str:
    """
...
def invoke_explorer(
    ...
    """
    Run an explorer against the report template using *llm*.
    ...
    return invoke_reviewer(
        llm, prompt, template_tree_check(template), build_correction_prompt
    )
