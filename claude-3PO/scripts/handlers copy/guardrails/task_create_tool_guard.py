...
Requires ``metadata.parent_task_id`` and ``metadata.parent_task_title`` and
confirms the parent ID matches a project task already loaded into state.
"""
...
class TaskCreateToolGuard:
    ...
    Ensures the new task is wired to a real project-manager task — both
    metadata fields are present *and* the parent ID is one of the loaded
    project tasks.
    ...
    def __init__(self, hook_input: dict, config: Config, state: StateStore):
        """
        Cache the hook payload, config, and state.
    ...
    def get_metadata(self) -> dict:
        ...
        Example:
            >>> metadata = guard.get_metadata()  # doctest: +SKIP
        """
        tool_input = self.hook_input.get("tool_input", {})
        return tool_input.get("metadata") or {}

    def check_parent_task_id(self, parent_task_id: str | None) -> None:
        """
        ...
        Example:
            ...
            >>> guard.check_parent_task_id(None)  # doctest: +SKIP
        """
    ...
    def check_parent_task_title(self, parent_task_title: str | None) -> None:
        """
        ...
        Example:
            ...
            >>> guard.check_parent_task_title(None)  # doctest: +SKIP
        """
    ...
    def check_parent_exists(self, parent_task_id: str) -> None:
        """
        ...
        Example:
            ...
            >>> guard.check_parent_exists("missing-id")  # doctest: +SKIP
        """
    ...
    def validate(self) -> Decision:
        ...
        Returns:
            Decision: ``("allow", message)`` if metadata is well-formed and
            the parent exists. Otherwise ``("block", reason)``.
        ...
        try:
            ...
            self.check_parent_task_id(parent_task_id)
            self.check_parent_task_title(parent_task_title)
            self.check_parent_exists(parent_task_id)
