...
Fuzzy-matches ``task_subject`` against the titles of ``project_tasks`` already
loaded from the project manager.

This guard never mutates state. After Allow, dispatchers read the matched
data from these instance attributes:

- ``matched_implement_parent_id`` — set with the parent project-task ID.
- ``matched_implement_payload`` — set with the dict that should be appended as
  a child of the parent project task.
"""
...
class TaskCreatedGuard:
    ...
    - ``matched_implement_parent_id``: ID of the project task that owns the
      new task.
    - ``matched_implement_payload``: payload dict that dispatchers should
      append as a child of the parent project task.
    ...
    def validate(self) -> Decision:
        ...
        On Allow the dispatcher should consume the ``matched_*`` attributes to
        apply state changes via Recorder.

        Returns:
            Decision: ``("allow", message)`` if the subject matches a known
            project task, otherwise ``("block", reason)``.
