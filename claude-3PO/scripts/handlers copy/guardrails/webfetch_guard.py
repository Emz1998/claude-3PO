...
class WebFetchGuard:
    ...
    def check_url_present(self) -> None:
        """
        Reject empty URLs.
        ...
        Example:
            ...
            >>> guard.check_url_present()  # doctest: +SKIP
        """
        if not self.url:
            raise ValueError("URL is empty")

    def check_hostname_parseable(self) -> str:
        """
        ...
        Example:
            >>> host = guard.check_hostname_parseable()  # doctest: +SKIP
        """
    ...
    def check_domain_safe(self, host: str) -> None:
        """
        ...
        Example:
            ...
            >>> guard.check_domain_safe("evil.example")  # doctest: +SKIP
        """
        # Accept exact match and single-segment subdomains — callers whitelist
        # bases like ``example.com`` expecting ``api.example.com`` to follow.
    ...
    def validate(self) -> Decision:
        ...
        """
        try:
            self.check_url_present()
            host = self.check_hostname_parseable()
            self.check_domain_safe(host)
