...
- Write results to `${CLAUDE_PLUGIN_ROOT}/E2E_TEST_REPORT.md` after each phase. This file is always writable in test mode.
...
2. `Edit` on any existing non-plan file (e.g. `${CLAUDE_PLUGIN_ROOT}/scripts/stop.py`) — _should block_ (only plan file editable)
...
2. `Edit` on any existing non-test file (e.g. `${CLAUDE_PLUGIN_ROOT}/scripts/stop.py`) — _should block_ (not a test file in session)
...
2. `Edit` on any existing non-code-session file (e.g. `${CLAUDE_PLUGIN_ROOT}/scripts/stop.py`) — _should block_ (not in session code files)
...
1. `Edit` on `src/hello.py`, old*string: `return "hello world"`, new_string: `return "hello world!"` — \_should block* (test-first enforcement: tests_to_revise must be edited before code files)
2. `Edit` on `test_hello.py`, old*string: `assert hello() is None  # revised`, new_string: `assert hello() is None  # code-review revised` — \_should allow*
3. `Edit` on `src/hello.py`, old*string: `return "hello world"`, new_string: `return "hello world!"` — \_should allow* (tests revised, code edit now permitted)
4. `Agent` with `subagent_type: "CodeReviewer"`, prompt below — _should allow_
...
| ... | revise-plan | Skill | revise-plan | ... |
| ... | code-review | Edit | src/hello.py (test-first enforcement) | ... |
```
...
1. `/ci-check` — _should block_ (pr-create not completed)
2. Invoke `/continue` to advance past pr-create
3. Invoke `/ci-check`
4. `Bash` with `echo "hello"` — _should block_ (not in CI commands list)
5. `Bash` with `gh pr checks` — _should block_ (missing --json flag)
...
```markdown
| ... | ci-check | Skill | ci-check (pr-create not done) | ... |
